...
async def run_benchmark(
    ...
    try:
        saged_service = SagedService()
        # Extract domain from the benchmark data
        if isinstance(config.benchmark, dict) and 'domain' in config.benchmark:
            domain = config.benchmark['domain']
            if isinstance(domain, dict) and '0' in domain:
                domain = domain['0']  # Get the first domain value
        else:
            raise HTTPException(status_code=400, detail="Invalid benchmark data: missing domain")
            
        return await saged_service.run_benchmark(domain, config)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) 